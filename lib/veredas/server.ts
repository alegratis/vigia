  ...
  const features: VeredasFeatureCollection["features"] = boundaries.map((boundary) => {
    ...
    return {
      ...
      properties: {
        ...
        historyDistanceKm: agg?.historyDistanceKm ?? null,
        rainfallRatio: agg?.rainfallRatio ?? null,
        floodScoreAvg: agg?.floodScoreAvg ?? null,
        floodLevel: agg?.floodLevel ?? null,
        floodStreamDistanceKm: agg?.floodStreamDistanceKm ?? null,
        floodZoningLevel: agg?.floodZoningLevel ?? null,
        floodZoningCovered: agg?.floodZoningCovered ?? false,
        fireScoreAvg: agg?.fireScoreAvg ?? null,
        fireLevel: agg?.fireLevel ?? null,
        fireHistoryCount: agg?.fireHistoryCount ?? null,
        fireFwi: agg?.fireFwi ?? null,
        seismicScoreAvg: agg?.seismicScoreAvg ?? null,
        seismicNearestEventKm: agg?.seismicNearestEventKm ?? null,
        seismicNearestMagnitude: agg?.seismicNearestMagnitude ?? null,
