...
interface CompoundVeredasOverlayProps {
  ...
  /** Municipalities toggled on via the shared MunicipioTogglePanel; others render dimmed. */
 ...
 */
export function CompoundVeredasOverlay({
  enabled,
  onSelect,
  onOpenReport,
  activeMunicipios,
}: CompoundVeredasOverlayProps) {
      ...
      {veredas.features.map((feature) => {
        ...
        return (
          <Polygon
            ...
            pathOptions={
              active
                ? { color: "#fff", weight: 1, opacity: 0.9, fillColor, fillOpacity: 0.65 }
                : {
                    color: noDataColor ?? "#888",
                    weight: 1,
                    opacity: 0.3,
                    fillColor: noDataColor ?? "#888",
                    fillOpacity: 0.12,
                  }
            }
            eventHandlers={{
